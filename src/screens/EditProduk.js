...
import {ubahProduk, getProdukDetail} from '../redux/actions/produk';
...
export default function TambahProduk({navigation, route}) {
  const dispatch = useDispatch();
  const {token} = useSelector((state) => state.auth);
  const {produkDetail} = useSelector((state) => state.produk);
  const [foto, setFoto] = React.useState(produkDetail.foto);
  const [vfoto, setVfoto] = React.useState(null);
  const [nama, setNama] = React.useState(produkDetail.nama);
  const [harga, setHarga] = React.useState(produkDetail.harga.toString());
  const [stok, setStok] = React.useState(produkDetail.stok.toString());
  const [deskripsi, setDeskripsi] = React.useState(produkDetail.deskripsi);
  ...
  const ubah = () => {
    ...
    dispatch(ubahProduk(token, formData, id));
      ...
      <ScrollView style={styles.container}>
        <View style={styles.produkCard}>
          <TouchableOpacity
            ...
            <Image
              source={vfoto === null ? {uri: imageURI + foto} : vfoto}
              style={styles.images}
            />
          </TouchableOpacity>
          <InputBorderedBottom
